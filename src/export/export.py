...
def export_to_onnx(model_path, device_config, output_dir, imgsz=None):
    ...
    if onnx_settings.get('simplify', False):
        print("🔧 Running additional ONNX simplification...")
        try:
            ...
            if check:
                ...
                print("⚠️  ONNX simplification validation failed, keeping original")
...
def compile_oakd(onnx_path, device_config, output_dir, imgsz=416):
    ...
    print(f"\n🔨 Compiling for OAK-D using ModelConverter...")
...
"""
    ...
    print(f"📋 Created ModelConverter config: {config_path}")
    
    try:
        import subprocess
        
        print("🔨 Running ModelConverter (this may take 1-3 minutes)...")
        ...
        if result.returncode != 0:
            print(f"❌ ModelConverter failed with return code: {result.returncode}")
            return None
        ...
        if not output_dirs:
            print("⚠️  Could not locate output directory")
            return None
        ...
        if not blob_path.exists():
            print(f"⚠️  Could not locate output blob in {latest_output}")
            return None
        ...
        print(f"✅ OAK-D .blob compiled: {final_blob}")
        return final_blob
            
    except ImportError:
        print("⚠️  modelconverter not installed")
        print("\n📝 Install with: pip install modelconv")
        return None
    except Exception as e:
        print(f"❌ Error running ModelConverter: {e}")
        return None


def compile_jetson(onnx_path, device_config, output_dir):
    """Instructions for compiling ONNX to TensorRT .engine on Jetson."""
    print(f"\n🔨 Preparing for Jetson TensorRT compilation...")
    print(f"   ⚠️  TensorRT engines MUST be compiled on the target device")
...
Examples:
  ...
  python export.py --model models/checkpoints/can_detector_pihat_v1/weights/best.pt --device jetson_orin_nano
    ...
    # Load device configuration
    print(f"🔍 Loading device config: {args.device}")
    ...
    try:
        ...
        if args.compile:
            ...
            print(f"\n💡 Tip: Add --compile flag to automatically compile to {args.device} format")
