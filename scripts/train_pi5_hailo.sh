...
# Downloads dataset, trains model, exports to ONNX (ready for Hailo compilation)
...
PROJECT_NAME="fcc4"
IMAGE_SIZE=640
MODEL_SIZE="n"  # nano model for edge devices
EPOCHS=100
...
usage() {
    ...
    echo "  -n RUN_NAME   Custom run name (optional, default: pi5_hailo_v{VERSION})"
    echo ""
    echo "Example:"
    echo "  $0 -v 4"
    echo "  $0 -v 4 -n my_can_detector"
    exit 1
}
...
model:
  size: "${MODEL_SIZE}"  # nano model for edge deployment
...
training:
  epochs: ${EPOCHS}
  imgsz: ${IMAGE_SIZE}  # Pi 5 + Hailo works well with 640x640
...
# Step 4: Export for Pi 5 + Hailo
echo -e "${YELLOW}📦 Step 4/4: Exporting model for Pi 5 + Hailo-8...${NC}"

EXPORT_DIR="models/exported/${RUN_NAME}/pi5_hailo8"
...
echo -e "   1. Compile ONNX to .hef using Hailo Docker:"
echo -e "      python docker/run_hailo_compile.py --onnx ${EXPORT_DIR}/best.onnx"
echo -e "   2. Deploy to Pi 5 + Hailo device (deployment script coming soon)"
echo ""
